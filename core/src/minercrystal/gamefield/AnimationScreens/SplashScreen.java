...
public class SplashScreen {
    float alphaSlpashScreenTimer = 0, alphaSlpashSpeed = 0.008f,
            SPLASH_ACCUMULATOR = 0.03f, STimer = 0, alphaBackroundTimer = 0, AnimationDelay = 0.035f, AnimationTimer = 0;
    Texture LogoTexture;
    ...
    public void setSplashScreen(float delta, SpriteBatch spriteBatch, OrthographicCamera camera, BackgroundScreen backgroundScreen) {
        Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
        spriteBatch.begin();

        if ((alphaBackroundTimer < 1 && isLogoActive) || backgroundActive) {
            STimer += delta;
            ...
            spriteBatch.setColor(c.r, c.g, c.b, alphaBackroundTimer); //set alpha to 1
            backgroundScreen.render(delta, spriteBatch);

            if (alphaBackroundTimer > 0.96) {
                backgroundActive = true;
                alphaBackroundTimer = 1;
            }
        }
